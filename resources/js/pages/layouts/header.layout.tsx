// src/layouts/header.layout.tsx (adaptive + portal popovers to avoid clipping)
...
// You can freely add/remove socials; UI adapts automatically
...
export default function HeaderLayout({ sections = [] }: HeaderLayoutProps) {
  ...
  // naive match-media without external deps
  ...
  // Allowed section chips = total budget minus number of socials, with floor & ceiling
  ...
  // Size & gap tuning based on combined length (sections + socials)
  ...
  return (
    <nav className="z-[60] w-full px-2 pt-7 md:pt-10 mb-14 flex justify-center">
      <div className="relative w-full max-w-5xl">
        ...
        <div className="hidden md:flex items-center gap-3 rounded-full border border-zinc-200 dark:border-zinc-700 bg-white/90 dark:bg-zinc-900/80 p-2 pl-6 pr-5 shadow-2xl backdrop-blur-xl transition-all overflow-visible">
          ...
          {/* Sections (inline) with horizontal scroll to prevent pushing socials */}
          ...
          <AnimatePresence>
            {open && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 16 }}
                className="absolute right-0 top-[110%] z-[70] w-72 rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white/90 dark:bg-zinc-900/95 p-3 shadow-2xl"
              >
