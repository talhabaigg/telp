...
/**
 * Sticky, glassy footer with:
 * - TELP logo on the left
 * - Mobile menu (animated dropdown)
 * - Desktop inline menu
 * - Theme toggle (dark/light)
 * - Copyright line
 *
 * The footer is pointer-events aware so it does not block the page,
 * while the inner container re-enables interaction.
 */
export default function FooterLayout(): JSX.Element {
    ...
    /** Unique id for the mobile menu region. */
    const mobileMenuId = useId();

    /** Current year, computed once. */
    const year = useMemo(() => new Date().getFullYear(), []);

    /** Footer navigation menus (memoized for i18n). */
        ...
        <footer className="pointer-events-none fixed bottom-0 left-0 z-40 w-full px-2 select-none">
            ...
            >
                ...
                <div className="mt-2 hidden w-full items-center justify-end gap-2 sm:flex sm:mt-0 sm:w-auto">
                    ...
                    <span className="ml-3 whitespace-nowrap text-xs text-white opacity-80">
                        © {year} {t("All rights reserved.")}
                    </span>
                </div>
            </motion.div>
